...
# pseudo_data_frame = data_frame.loc[data_frame["condition"] == "pseudo"].loc[data_frame["participant"].isin(participants)]

df_cog_x = data_frame[["map_number", 'x_cog', 'y_cog',"muscle", 'participant', "condition"]].loc[data_frame["participant"].isin(participants)]


#compute euclidian distance between cogs for each condition and each map
err_euclid = []
for map in df_cog_x[df_cog_x['condition'] == 'grid']['map_number'].unique():
    cog_pseudo = np.array(df_cog_x[df_cog_x['condition'] == 'pseudo'][df_cog_x['map_number'] == map + 1][['x_cog', 'y_cog']])
    cog_grid = np.array(df_cog_x[df_cog_x['condition'] == 'grid'][df_cog_x['map_number'] == map][['x_cog', 'y_cog']])
    distance = np.sqrt(np.sum((cog_pseudo - cog_grid)**2, axis=1))
    err_euclid.append(distance)
