...
def main(results_dir):
    ...
    for i, participant in enumerate(min_maps["participant"].unique()):
        for j, muscle in enumerate(min_maps["muscle"].unique()):
            for c, cond in enumerate(["grid", "pseudo"]):
                min_map = min_maps.loc[
                    (min_maps["participant"] == participant)
                    & (min_maps["muscle"] == muscle)
                    & (min_maps["condition"] == cond)
                ]["min_map_number"].values[0]
                if not np.isfinite(min_map):
                    continue
                pd_tmp = data_frame.loc[
                    (data_frame["participant"] == participant)
                    & (data_frame["muscle"] == muscle)
                    & (data_frame["condition"] == cond)
                    & (data_frame["map_number"] == list_points_tot[c].index(min_map))
                ]
    ...
    col = [[(0, 0), (0, 3)], [(0, 0), (0, 3)], [(1, 0), (1, 3)], [(1, 0), (1, 3)]]
    ...
    for r, rating in enumerate(["x_cog", "y_cog", "area", "normalize_volume"]):
        ...
        for c, cond in enumerate(["grid", "pseudo", "pseudo"]):
            ...
            for m, map in enumerate(list_points):
                pseudo = data_frame.loc[data_frame["condition"] == cond].loc[
                    data_frame["map_number"] == list_points.index(map)
                ]
                pseudo["ref"] = 0
                if m == 0:
                    ...
                    min_map_tmp["ref"] = 0
                if c == 2:
                    ref = data_frame.loc[data_frame["condition"] == "grid"].loc[
                        data_frame["map_number"] == list_points_tot[0].index(list_points_tot[0][-1])
                    ]
                else:
                    ref = data_frame.loc[data_frame["condition"] == cond].loc[
                        data_frame["map_number"] == list_points_tot[c].index(list_points_tot[c][-1])
                    ]
                ref["ref"] = 1
                merged = pseudo.reset_index(drop=True).merge(
                    on=["participant", "muscle"], right=ref.reset_index(drop=True)
                )
                ...
                for mus in merged["muscle"].unique():
                    if rating == "cog":
                        ...
                        rmse = np.sqrt(
                            np.nanmean(
                                (
                                    concat_pd.loc[concat_pd["muscle"] == mus].loc[concat_pd["ref"] == 1][rating].values
                                    - concat_pd.loc[concat_pd["muscle"] == mus]
                                    .loc[concat_pd["ref"] == 0][rating]
                                    .values
                                )
                                ** 2
                            )
                    ...
                    if rating == "area":
                        rmse = rmse * 100 / 3600
                    dict = {"RMSE": [rmse], "rating": [rating], "map": [map], "comp": [comp[c]], "muscle": [mus]}
                ...
                if m == 0:
                    ...
                    for mus in concat_pd["muscle"].unique():
                        ...
                        if rating == "cog":
                            ...
                            y_ref = concat_pd.loc[concat_pd["muscle"] == mus].loc[concat_pd["ref"] == 1]["y_cog"].values
                            x_pseudo = (
                                concat_pd.loc[concat_pd["muscle"] == mus].loc[concat_pd["ref"] == 0]["x_cog"].values
                            )
                            y_pseudo = (
                                concat_pd.loc[concat_pd["muscle"] == mus].loc[concat_pd["ref"] == 0]["y_cog"].values
                            )
                            rmse = np.nanmean(np.sqrt((x_pseudo - x_ref) ** 2 + (y_pseudo - y_ref) ** 2))
                        else:
                        ...
                        if rating == "area":
                            rmse = rmse * 100 / 3600
                        dict = {"RMSE": [rmse], "rating": [rating], "map": ["min"], "comp": comp[c], "muscle": mus}
