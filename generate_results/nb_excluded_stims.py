...
if __name__ == "__main__":
    sci = False
    result_dir = r"D:\Documents\Programmation\tms_motor_map\results\smooth_8_4_5" if not sci else "sci_smooth_10_5_2543"
    ...
    for i in range(len(data_maps)):
        data_frame_tmp = pd.DataFrame(
            {
                "nb_excluded_stims": [
                    np.count_nonzero(np.isnan(da)) / da.shape[0] * 100 for da in data_maps[i]["z_list"]
                ],
