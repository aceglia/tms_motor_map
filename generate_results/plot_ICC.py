...
if __name__ == '__main__':
    results_dir = r'D:\Documents\Programmation\tms_motor_map\results\smooth_7_5_10'
    ...
    for r, rating in enumerate(['x_cog', 'y_cog', 'area', 'volume']):
        ...
        for c, cond in enumerate(['grid', 'pseudo', 'pseudo']):
            ...
            for m, map in enumerate(list_points):
                ...
                icc_df = pg.intraclass_corr(data=pd.concat([ref, pseudo], ignore_index=True), targets='participant', raters='ref', ratings=rating).round(2)
                ...
                if m == 0:
                    icc_df_min = pg.intraclass_corr(data=pd.concat([ref, min_map_tmp], ignore_index=True), targets='participant', raters='ref', ratings=rating).round(2)
        ...
        for i in range(2):
            ...
            if i == 0:
                ...
                ax = axes_tmp[i]
                sns.lineplot(x='map', y='ICC', hue='comp', data=icc3, ax=ax, marker='o', palette="rocket",legend=r==0)
                ax.set_title('Overall correlation', fontsize=12)
                if r==0:
                ...
                axes_tmp.append(fig_tot.add_subplot(gs[:, loc_tmp[1]:loc_tmp[1]+span_tmp]))
                ax = axes_tmp[i]
                sns.barplot(x='comp', y='ICC', data=icc3_min, ax=ax, palette="rocket")
                ax.set_title('Optimal map ICC', fontsize=12)
