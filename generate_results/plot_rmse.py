        ...
        for c, cond in enumerate(["grid", "pseudo", "pseudo"]):
            ...
            for mus in concat_pd["muscle"].unique():
                ...
                if rating == "cog":
                    ...
                    rmse_tmp = np.sqrt(
                        np.mean(
                            ref.loc[ref["muscle"] == mus][rating].values
                            - min_map_tmp.loc[min_map_tmp["muscle"] == mus][rating].values
                        )
                        ** 2
                    )
                if rating == "area":
                    rmse_tmp = rmse_tmp * 100 / 3600
                rmse_tmp_dic = {"rmse": [rmse_tmp], "muscle": [mus], "rating": [rating], "comp": [comp[c]]}
