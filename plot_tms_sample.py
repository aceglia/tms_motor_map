...
def project_points_to_plane(points):
    ...
    xx, yy = np.meshgrid(
        np.linspace(points[:, 0].min(), points[:, 0].max(), 10), np.linspace(points[:, 1].min(), points[:, 1].max(), 10)
    )
    normal = (C[0], C[1], -1)
...
def least_square_plane(points, target_names):
    ...
    ax.scatter(
        projected_points[:, 0], projected_points[:, 1], projected_points[:, 2], color="g", label="Projected Points"
    )

    ax.plot_surface(xx, yy, zz, alpha=0.5, label="Fitted Plane")
    # plot x, y, z axis
    ax.quiver(origin[0], origin[1], origin[2], x_axis[0], x_axis[1], x_axis[2], color="r", label="X-axis", length=10)
    ax.quiver(origin[0], origin[1], origin[2], y_axis[0], y_axis[1], y_axis[2], color="g", label="Y-axis", length=10)
    ax.quiver(
        origin[0], origin[1], origin[2], z_axis[0], z_axis[1], z_axis[2], color="b", label="Z-axis", length=10
...
if __name__ == "__main__":
    ...
    for t in trials:
        ...
        if idx_wrong:
            ...
            data[idx_wrong[0]]["signal_data"]["data"] = np.concatenate(
                [data[idx_wrong[0]]["signal_data"]["data"], np.zeros((to_fill, 6, 1))], axis=0
            )
